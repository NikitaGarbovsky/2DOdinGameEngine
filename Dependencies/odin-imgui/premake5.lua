...
local function setupPythonEnvironment()
    -- Hardcode Python path (adjust to your install)
    local python_exe = "C:\\Users\\brusa\\AppData\\Local\\Programs\\Python\\Python314\\python.exe"
    -- Optional quick test
    if not os.isfile(python_exe) then
        error("Hardcoded Python executable not found: " .. python_exe)
    end

    VENV_DIR = path.translate(BUILD_DIR .. "/venv")
    if not os.isdir(VENV_DIR) then
        print("Creating Python virtual environment...")
        if not os.execute('"' .. python_exe .. '" -m venv ' .. VENV_DIR) then
            error("Failed to create virtual environment.")
        end
    end

    local python = path.translate(VENV_DIR .. "/Scripts/python.exe")
    local pip = path.translate(VENV_DIR .. "/Scripts/pip.exe")
